...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(
            ...
            ).SelectMany(empDept => grades.Where(
            s => empDept.emp.Sal >= s.Losal && empDept.emp.Sal <= s.Hisal),
            (empDept, s) => new{ EName = empDept.emp.EName, DName = empDept.dept.DName, Grade = s.Grade }).ToList();
